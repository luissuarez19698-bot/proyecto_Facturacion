        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
          ...
          <div className="md:col-span-4 flex flex-row gap-2">
            <div className="flex flex-1 items-center justify-between bg-gray-50 rounded-lg p-1 border border-gray-100">
              <button onClick={decrementar} disabled={cantidad <= 1 || !productoId} className="w-9 h-9 flex items-center justify-center rounded-md bg-white shadow-sm hover:bg-gray-100 disabled:opacity-30 transition-all text-emerald-700 font-black"> − </button>
              <span className={`text-sm font-black ${esCantidadInvalida && productoId ? 'text-red-600' : 'text-gray-800'}`}>{cantidad}</span>
              <button onClick={incrementar} disabled={cantidad >= stockReal || !productoId} className="w-9 h-9 flex items-center justify-center rounded-md bg-white shadow-sm hover:bg-gray-100 disabled:opacity-30 transition-all text-emerald-700 font-black"> + </button>
            </div>
